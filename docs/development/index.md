...
2. Add the key to `packages/frontend/src/i18n/en.ts` and to every other language file (`tr.ts`, `es.ts`, `de.ts`, `fr.ts`)
3. English (`en.ts`) is the type source — all other language files must match its shape
