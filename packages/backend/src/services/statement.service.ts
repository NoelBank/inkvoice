...
const LABELS: Record<"en" | "tr" | "es" | "de" | "fr", Record<string, string>> = {
  en: {
    statement: "Statement",
  ...
  es: {
    ...
    charges: "Cargos",
    payments_credits: "Pagos / Abonos",
    balance: "Saldo",
    opening_balance: "Saldo inicial",
    closing_balance: "Saldo final",
    no_activity: "Sin movimientos en este período.",
    aging_summary: "Resumen de antigüedad (pendiente)",
    ...
    current: "No vencido",
    footer_note:
      "Este extracto refleja los movimientos del período seleccionado. Contáctenos si tiene alguna pregunta.",
  },
  de: {
    ...
    charges: "Belastungen",
    payments_credits: "Zahlungen / Gutschriften",
    balance: "Saldo",
    opening_balance: "Anfangssaldo",
    closing_balance: "Endsaldo",
    no_activity: "Keine Bewegungen in diesem Zeitraum.",
    aging_summary: "Fälligkeitsübersicht (offen)",
    total_due: "Fälliger Gesamtbetrag",
    invoice: "Rechnung",
    credit_note: "Gutschrift",
    payment: "Zahlung",
    current: "Nicht fällig",
    footer_note:
      "Dieser Auszug zeigt die Bewegungen im ausgewählten Zeitraum. Bei Fragen kontaktieren Sie uns bitte.",
  },
  fr: {
    ...
    current: "Non échu",
    footer_note:
      "Ce relevé reflète l'activité de la période sélectionnée. N'hésitez pas à nous contacter pour toute question.",
  },
};
...
export function buildStatementData(
  ...
  const langPrefix = (localeOverride || "en").slice(0, 2).toLowerCase();
  const lang = (langPrefix in LABELS ? langPrefix : "en") as keyof typeof LABELS;
